 ...
 * @since 1.0-SNAPSHOT
 */
class SpawnNexus() : Action<SpawnReport> {
    ...
    override val opcode: String = "spawn:nexus"


    override fun perform(agent: Agent, server: Server, world: World): SpawnReport {

        val dest = agent.location.clone()
        val agent: Nexus = Nexus(agent.player, randomId(), dest)
        dest.block.type = Material.REDSTONE_BLOCK
        agent.player.agents[agent.id] = agent
        world.spawnParticle(Particle.HEART, agent.location, 20)
        return SpawnReport(reportId, agent.id, agent.player.login, agent.codename(), status(dest.block))
